<?php
$query = "select * FROM abacus2.scheduled_query WHERE next_run_dt<now() AND active=1 AND is_deleted=0";
...
$db[$abacus_db]->query("UPDATE abacus2.scheduled_query SET last_run_dt=next_run_dt WHERE scheduled_query_id=?", $job["scheduled_query_id"]);		

$db[$abacus_db]->query("UPDATE abacus2.scheduled_query SET next_run_dt=DATE_ADD(next_run_dt, $interval) WHERE scheduled_query_id=?", $job["scheduled_query_id"] );		
